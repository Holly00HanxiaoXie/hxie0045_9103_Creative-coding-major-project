...
//Creates an object that can be shared across all team members' code
//allowing all patterns to use the same set of settings. 
...
class CircularPattern {
  ...
  // Draw radial lines to create a bicycle wheel spoke effect
  ...
  drawConcentricCircle(diameter, fillColor, strokeColor = null, strokeWeight = 0) {
    fill(fillColor); //Skip border if no stroke color is se
    if (strokeColor) {
      stroke(strokeColor);
...
class PurplePattern extends CircularPattern {
  draw() {
   ...
   // color(...fill) uses the spread operator (...)
   // which takes each element in the array individually and passes it to the color() function. 

   // The reason for this is that the color() function requires individual RGB values ​​as arguments
   // while fill is an array containing RGB values.
...
class SecondPurplePattern extends CircularPattern {
  draw() {
    ...
    // Inner circle series
    const innerCircles = [
    // Define an array of circle objects, each containing the size
    // fill color, and stroke color for concentric circles
  ...
  // Iterate over each circle object in the innerCircles array
    ...
    ringRadii.forEach((radius, index) => {
      // Determine the size of each circle in the ring 
      // If the index is even, set circleSize to 5; if odd, set it to 10
...
class PatternManager {
  ...
  checkOverlap(x, y, size) {
    for (let pattern of this.patterns) {
      // Calculate the horizontal and vertical distance between the current position 
      // and the center point of the pattern
  ...
  createPatterns() {
    ...
    // Cycle pattern generation when the number of patterns does not reach the maximum 
    // and the number of attempts is not exceeded
      ...
      if (!this.checkOverlap(x, y, size)) {
       //Select a random pattern class from the pattern class array
        ...
        const scale = size / 350;
         ...
         // passing in the randomly generated position and scale
  ...
  draw() {
    // Sort by pattern size in descending order
    // with larger patterns drawn at the bottom
